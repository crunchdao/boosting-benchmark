##import Pkg;
##Pkg.add("CSV") ##Pkg.add("DataFrames") ##Pkg.add("UrlDownload") ##Pkg.add("Arrow") ##Pkg.add("LightGBM") ##Pkg.add("XGBoost") ##Pkg.add("EvoTrees")

@info "Relative paths assume that the folder /src is set as working directory. If data saving or loading does not work, then a wrongly set working directory might be the reason."

using DataFrames, Arrow, CSV, UrlDownload, Plots, Dates
...
if use_example_data
    is_download = false
    @info "use_example_data set to $(use_example_data). The example data from the round that opened on April 21, 2023 will be used."
end

is_test = false ## if this is true, then only 10% of train data is used to speed up tests
...
function generate_light_gbm_preds(current_target::AbstractString)
    println("*** Timing training of lightgbm on target: ", current_target, " ***")
    @time LightGBM.fit!(lightGBM_model, Matrix(convert.(T, train_data[train_indices, Cols(x -> contains(x, "Feature"))])), train_data[train_indices, current_target]; verbosity = -1) 
    println("*** End of timing training of lightgbm on target: ", current_target, " ***")

    ## predict train
    lightgbm_pred_train = LightGBM.predict(lightGBM_model, Matrix(convert.(T, train_data[train_indices, Cols(x -> contains(x, "Feature"))])));
    ...
    ## validation
    lightgbm_pred_val = LightGBM.predict(lightGBM_model, Matrix(convert.(T, train_data[val_indices, Cols(x -> contains(x, "Feature"))])));
    ...
    ## predict test
    lightgbm_pred_live = LightGBM.predict(lightGBM_model, Matrix(convert.(T, test_data[:, Cols(x -> contains(x, "Feature"))])));
...
function generate_xgboost_preds(current_target::AbstractString)
    println("*** Timing training of xgboost on target: ", current_target, " ***")
    @time xgboost_model = xgboost( DMatrix(convert.(T, train_data[train_indices, Cols(x -> contains(x, "Feature"))]), train_data[train_indices, current_target]), 
    ...
    println("*** End of timing training of xgboost on target: ", current_target, " ***")

    # train, rescaled to be in [0,1]
    xgboost_pred_train = XGBoost.predict(xgboost_model, DMatrix(convert.(T, train_data[train_indices, Cols(x -> contains(x, "Feature"))]))) |> rescale;
   
    train_cor = get_cor_measure(xgboost_pred_train, train_data[train_indices, current_target]);
    println("Train performance ", current_target, " : ", train_cor[1,1])

    # validation
    xgboost_pred_val = XGBoost.predict(xgboost_model, DMatrix(convert.(T, train_data[val_indices, Cols(x -> contains(x, "Feature"))]))) |> rescale;
    ...
    # test        
    xgboost_pred_live = XGBoost.predict(xgboost_model, DMatrix(convert.(T, test_data[:, Cols(x -> contains(x, "Feature"))]))) |> rescale;
...
function generate_evotree_preds(current_target::AbstractString)
    println("*** Timing training of evotree on target: ", current_target, " ***")
    @time m_evo = fit_evotree(params_evo; x_train = Matrix(convert.(T, train_data[train_indices, Cols(x -> contains(x, "Feature"))])), y_train = train_data[train_indices, current_target], metric=:mae, print_every_n=500, 
                             x_eval = Matrix(convert.(T, train_data[train_indices, Cols(x -> contains(x, "Feature"))])), y_eval = train_data[train_indices, current_target]);
    println("*** End of timing training of evotree on target: ", current_target, " ***")
    
    evotree_pred_train = EvoTrees.predict(m_evo, Matrix(convert.(T, train_data[train_indices, Cols(x -> contains(x, "Feature"))])));
    train_cor = get_cor_measure(evotree_pred_train, train_data[train_indices, current_target]);
    println("Train performance ", current_target, " : ", train_cor[1,1])

    ## validation
    evotree_pred_val = EvoTrees.predict(m_evo, Matrix(convert.(T, train_data[val_indices, Cols(x -> contains(x, "Feature"))])));
    val_cor = get_cor_measure(evotree_pred_val, train_data[val_indices, current_target]);
    println("Validation performance ", current_target, " : ", val_cor[1,1])

    ## predict test
    evotree_pred_live = EvoTrees.predict(m_evo, Matrix(convert.(T, test_data[:, Cols(x -> contains(x, "Feature"))])));
...
if is_download & !use_example_data
    ...
    train_data |> Arrow.write(relpath("..\\data\\"*file_name_train))
    ##test_data |> Arrow.write(joinpath(pwd(),"..\\data", file_name_test)) or alternative below, assuming you call the script from folder "src"
    test_data |> Arrow.write(relpath("..\\data\\"*file_name_test))

    print("\n data is downloaded")
else  
    if !use_example_data
        train_data = read_data(relpath("..\\data\\"*file_name_train))
        test_data = read_data(relpath("..\\data\\"*file_name_test))
    else     
        train_data = read_data(relpath("..\\data\\"*file_name_train_example))
        test_data = read_data(relpath("..\\data\\"*file_name_test_example))
    end
end
...
## using a symbol to select a column
...
@info "\n Train data with: $max_moon moons."
...
if use_example_data
    exportname_full = "results_table_full_example"
    exportname_w = "results_table_w_example"
else 
    d = Dates.format(Dates.today(), "yyyy-mm-dd")
    exportname_full = "results_table_full_current"*" "*d
    exportname_w = "results_table_w_current"*" "*d
end

CSV.write(relpath("..//"*exportname_full*".csv"), res_table)
CSV.write(relpath("..//"*exportname_w*".csv"), sort(filter(:target => ==("target_w"), res_table), :valcor, rev=true))
